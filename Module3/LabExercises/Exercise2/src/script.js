let question_1 = `1. Create a function that takes a string as a parameter and returns the string with the first
...
let question_2 = `2. Create a function truncate(str, max) that truncates a given string of text if its total
...
function truncate(str, max) {
    ...
    // a) Method1: simple if else condition
    // if(str.length > max) {
    ...
    // b) Method2: with conditional operators
...
let question_3 = `3. Use the following animals array for the below tasks. Test each one by printing the result to
...
let question_4 = `4. Write a function camelCase(cssProp) that changes dash-separated CSS properties like
...
let question_5 = `5. Decimal number operations in JavaScript can lead to unexpected results, as in the
following:
...
// c) creating function which will perform mathematical function provided with operation
...
let question_6 = `6. Create a function unique(duplicatesArray) which takes an array parameter that may
...
function unique(duplicatesArray) {
    ...
    return [...new Set(duplicatesArray)]; // ... will create new array
...
let question_7 = `7. Use the following array of book objects to practice the array functions for map, find and
